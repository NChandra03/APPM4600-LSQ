...
def driver():
  ...
  for i, n in enumerate(degrees):
    ...
    axes[i, 0].set_title(r"$f(x) = x^5 - 3x^3 + 5x$ with noise in $x$ and $y$, degree " + f"{n}" + " polynomial approx)
    ...
    axes[i, 1].set_title(r"$f(x) = x e^{-x^2}$ with noise in $x$ and $y$, degree " + f"{n}" + " polynomial approx")
  ...
  for idx, sigma in enumerate(noise_levels):
    ...
    axes[idx, 0].set_title(r"$f(x) = x^5 - 3x^3 + 5x$ with noise in $x$ and $y$, Noise Level " + f"{sigma}")
    ...
    axes[idx, 1].set_title(r"$f(x) = x e^{-x^2}$ with noise in $x$ and $y$, Noise Level " + f"{sigma}")

  # Save the plot
  plt.tight_layout()
  plt.savefig("xnoise_compare_noise_levels.png", dpi=300)
  plt.show()
