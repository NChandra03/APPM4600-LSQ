...
def driver():
  ...
  for idx, sigma in enumerate(noise_levels):
    ...
    # add smoothing using kernel smoothing function
    x_smooth1, fex_smooth1 = kernel_smooth_data(xeval, fex_noise1, bandwidth=(b-a)/10)
    x_smooth2, fex_smooth2 = kernel_smooth_data(xeval, fex_noise2, bandwidth=(b-a)/10)
  ...
  plt.tight_layout()
  plt.savefig("kernel_compare_lsr.png", dpi=300)
  plt.show()
...
def kernel_smooth_data(xi, yi, bandwidth):
    ...
    for i in range(len(xi)):
        ...
        for j in range(len(xi)):
            weight = gaussian_kernel(xi[j] - xi[i], bandwidth)
